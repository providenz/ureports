...
def rides(request):
    ...
    if query:
        rides_list = rides_list.filter(Q(car__mark__icontains=query) | Q(car__number__icontains=query))
    else:
        query = ""
    if date_from_b:
    ...
    return render(
        request,
        "road_logistics/rides.html",
        {"rides": rides_list, "q": query, "date_from": date_from_b, "date_to": date_to_b},
    )
...
class FetchKoboAPI(APIView):
    def post(self, request, *args, **kwargs):
        ...
        end_odometer_content, end_odometer_name = search_kobo_image(
            data["group_finish/photo_odometr_end"], attachments
        )

        is_refueling = True if data["group_finish/_"] == "yes" else False
        if is_refueling:
            refueling_check_content, refueling_check_name = search_kobo_image(
                data["group_finish/photo_check"], attachments
            )
        ...
        for x in range(1, 6):
            if f"group_start/_point_{x}" in data.keys():
                ...
                RidePoint.objects.create(
                    number=x, ride=ride, point=GEOSGeometry(f"POINT({point_coords[1]} {point_coords[0]})")
                )
            else:
                break
